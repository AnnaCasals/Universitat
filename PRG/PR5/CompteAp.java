...
public class CompteAp extends Compte implements Serializable {
    ...
    public void retirar(double quantitat) {
        ...
        apunts.encuar(new Apunt(Apunt.REINTEGRAMENT, quantitat * -1));
    }
     ...
     */
    public int getNumApunts() { return apunts.talla(); } 
    ...
    public String ultimsApunts(int k) {
        /* COMPLETAR */
        Apunt[] aux = apunts.toArray();
        int talla = aux.length;
        double quant = 0;
        String res = "Num. Apunts: " + k + "/" + getNumApunts() +"\n";
        for(int j=0; j < talla - k; j++) {quant += aux[j].getQuantitat();}
        for(int i= talla - k; i < talla; i++){
            quant += aux[i].getQuantitat();
            res += aux[i].toString() + 
            String.format(Locale.US, "  Saldo: %10.2f\n", quant); 
        }
    ...
    public String apunts() {
        return ultimsApunts(apunts.talla());
    }
